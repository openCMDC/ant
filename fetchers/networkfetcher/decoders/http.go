...
import (
	...
	"io"
	"mantis/fetchers/networkfetcher"
	"net/http"
	"regexp"
	"time"
)
...
func init() {
	...
	networkfetcher.RegisterDecoder("http", NewHttpDecoder)
}

type httpDecoder struct {
}

func (h *httpDecoder) Parse(ctx context.Context, conn *networkfetcher.TCPConn) {
	...
	go func() {
		for {
			...
			if err != nil {
				if err == networkfetcher.InternalReaderNil {
...
func NewHttpDecoder() networkfetcher.Interface {
