...
func (s *ServerEndPoint) startParseTcpConn(conn *TCPConn) {
	...
	go func() {
		<-s.decoderReady
		s.mutex.RLock()
		GetDecoder(s.decoderName).Parse(nil, conn)
		s.mutex.RUnlock()
	}()
}
...
func (sds *SingleDirectionStream) getInfo() (cAddr, sAddr *net.TCPAddr, st StreamType, err error) {
	...
	srcAddr, err := ParseIpAndPort2TCPAddr(netLayer.Src().String(), transLayer.Src().String())
	if err != nil {
		log.WithField("reason", err.Error()).Warn("parser to tcp addr failed")
		return nil, nil, -1, err
	}

	dstAddr, err := ParseIpAndPort2TCPAddr(netLayer.Dst().String(), transLayer.Dst().String())
	if err != nil {
		log.WithField("reason", err.Error()).Warn("parser to tcp addr failed")
		return nil, nil, -1, err
	}
	clientAddr, serverAddr := distinguishStreamDrc(srcAddr, dstAddr, sds.streamFactory.capture.addr)
	if clientAddr == nil || serverAddr == nil {
		log.WithFields(log.Fields{"srcAddr": srcAddr.String(), "dstAddr": dstAddr.String(), "addrs": sds.streamFactory.capture.addr}).Warn("can't determine client server addr")
		return nil, nil, -1, err
	}
	if clientAddr == srcAddr {
